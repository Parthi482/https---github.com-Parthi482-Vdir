...
@Output('onClick') onClick = new EventEmitter<any>();
  ...
  ngOnInit(): void {
    ...
    // this.latlong.push(location   ) 
    this.locationService.api.then((maps:any) => {
      this.initAutocomplete(maps);
      this.initMap(maps);
    });
...
this.searchLocation()
 ...
 marker.addListener("dragend", () => {
  ...
  if(this.latlong!=null&& this.latlong!=undefined&& this.mapShowFlag==false){
    ...
    // console.log(latlong.Latitude);
    // console.log(latlong.Longitude);
     let url=`https://maps.googleapis.com/maps/api/geocode/json?latlng=${location.Longitude},${location.Latitude}&key=${GOOGLE_MAPS_API_KEY}`


    center=new google.maps.LatLng( location.Longitude,location.Latitude);
    console.log(url);
